...
interface Coparticipacao {
  mes: number;
  data_utilizacao: string | null;
  coparticipacao_itens: CopartItem[];
}
...
const MinhaArea = () => {
  ...
  const getMensalidadeMes = (mes: number) => {
    const m = mensalidades.find((x) => x.mes === mes);
    return m ? m.valor : 0;
  };

  const getCopartMes = (mes: number) => {
    const items = coparticipacoes.filter((c) => c.mes === mes);
    return items.reduce((sum, c) => sum + (c.coparticipacao_itens || []).reduce((s, i) => s + i.valor, 0), 0);
  };

  const totalMensalidades = mensalidades.reduce((s, m) => s + m.valor, 0);
  const totalCopart = coparticipacoes.reduce(
    (s, c) => s + (c.coparticipacao_itens || []).reduce((s2, i) => s2 + i.valor, 0),
    0
  );
      ...
      <div className="min-h-screen bg-background p-8 print:p-4 print:bg-white">
        <div className="max-w-3xl mx-auto">
          ...
          <div className="border p-8 print:border-none print:p-0">
            ...
            <p className="text-center text-sm text-muted-foreground print:text-black mb-6">
              Ano-Calendário {ano}
            </p>
            ...
            <div className="grid grid-cols-2 gap-4 mb-6 text-sm">
              <div>
                ...
                <p>HAPVIDA ASSISTÊNCIA MÉDICA LTDA</p>
                <p>CNPJ: {HAPVIDA_CNPJ}</p>
              </div>
              <div>
                <p className="font-semibold">Beneficiário:</p>
                <p>{nome}</p>
                <p>CPF: {formatCpf(userCpf)}</p>
              </div>
            </div>
            ...
            <table className="w-full text-sm border-collapse">
              <thead>
                <tr className="border-b-2 border-foreground print:border-black">
                  ...
                  <th className="text-right py-2">Mensalidade</th>
                  <th className="text-right py-2">Coparticipação</th>
                  <th className="text-right py-2">Total</th>
                </tr>
              </thead>
              <tbody>
                {Array.from({ length: 12 }, (_, i) => i + 1).map((mes) => {
                  const mensal = getMensalidadeMes(mes);
                  const copart = getCopartMes(mes);
                  const total = mensal + copart;
                  if (total === 0) return null;
                  return (
                    <tr key={mes} className="border-b">
                      <td className="py-1">{MESES[mes - 1]}/{ano}</td>
                      <td className="text-right py-1">{fmt(mensal)}</td>
                      <td className="text-right py-1">{fmt(copart)}</td>
                      <td className="text-right py-1 font-medium">{fmt(total)}</td>
                    </tr>
                  );
                })}
              </tbody>
              <tfoot>
                <tr className="border-t-2 border-foreground print:border-black font-bold">
                  <td className="py-2">TOTAL</td>
                  <td className="text-right py-2">{fmt(totalMensalidades)}</td>
                  <td className="text-right py-2">{fmt(totalCopart)}</td>
                  <td className="text-right py-2">{fmt(totalMensalidades + totalCopart)}</td>
                </tr>
              </tfoot>
            </table>

            <p className="text-xs text-muted-foreground print:text-gray-600 mt-6">
              Documento gerado para fins de declaração de Imposto de Renda.
            </p>
          </div>
        </div>
      </div>
        ...
        <Card className="w-full max-w-md">
          <CardHeader className="text-center">
            ...
            <p className="text-sm text-muted-foreground">
              Acesse seus dados de plano de saúde
            </p>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            <div className="text-center">
              <NavLink to="/" className="text-sm text-muted-foreground hover:text-primary">
                Área administrativa
              </NavLink>
            </div>
          </CardContent>
  ...
  return (
    <div className="min-h-screen bg-background">
      <header className="border-b bg-card">
        <div className="container mx-auto px-4 py-4 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <Activity className="h-6 w-6 text-primary" />
            <h1 className="text-xl font-bold text-foreground">Minha Área</h1>
          </div>
      ...
      <main className="container mx-auto px-4 py-6">
        <Card>
          <CardHeader>
            <CardTitle className="text-lg">Resumo Anual - {ano}</CardTitle>
          </CardHeader>
          <CardContent>
                    ...
                    return (
                      <TableRow key={mes}>
                        ...
                        <TableCell className="text-right font-medium">
                          {mensal + copart > 0 ? fmt(mensal + copart) : "-"}
                        </TableCell>
                      </TableRow>
                    );
                  })}
